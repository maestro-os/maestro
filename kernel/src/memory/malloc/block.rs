...
impl Block {
	...
	pub fn new(min_size: NonZeroUsize) -> AllocResult<NonNull<Self>> {
		...
		// Allocate the block
		unsafe {
			let mut block = buddy::alloc_kernel(block_order)?.cast();
			block.write_volatile(Self {
				order: block_order,
				first_chunk: Chunk::new(),
			});
			*block.as_mut().first_chunk.as_free_chunk().unwrap() =
				FreeChunk::new(first_chunk_size);
			Ok(block)
		}
	}
	...
	pub unsafe fn from_first_chunk(chunk: NonNull<Chunk>) -> NonNull<Block> {
		let ptr = chunk.byte_sub(offset_of!(Block, first_chunk)).cast();
