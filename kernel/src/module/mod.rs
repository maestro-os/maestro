...
// Generated by build.rs: defines `BUILTIN_MODULES: &[&[u8]]`
include!(concat!(env!("OUT_DIR"), "/builtin_modules.rs"));

/// Loads all modules that were compiled into the kernel binary at build time.
///
/// The list of built-in module images is generated by `build.rs` into `builtin_modules.rs`.
/// Each image is loaded using the existing [`Module::load`] infrastructure.
pub fn load_builtin_modules() {
	for image in BUILTIN_MODULES {
		match Module::load(image) {
			Ok(m) => {
				if let Err(e) = add(m) {
					println!("Failed to add built-in module: {e}");
				}
			}
			Err(e) => println!("Failed to load built-in module: {e}"),
		}
	}
}
