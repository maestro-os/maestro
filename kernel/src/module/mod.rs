...
impl Module {
	...
	/// Creates a module from a built-in (statically compiled) init/fini pair.
	///
	/// Unlike [`Module::load`], this does not parse any ELF: the module code is
	/// already compiled into the kernel binary. `init` is called immediately; an
	/// error is returned if it returns `false`.
	pub fn new_builtin(
		name: &str,
		version: Version,
		...
		let name = name.try_into().inspect_err(|_| fini())?;
		Ok(Self {
			name,
			version,
			deps: Vec::new(),
...
// Generated by build.rs: declares `#[path]` modules for each built-in and
// defines `load_builtin_modules()`.
...
mod test {
	...
	extern "C" fn init_ok() -> bool {
		true
	}
	extern "C" fn init_fail() -> bool {
		false
	}
	extern "C" fn fini_noop() {}

	#[test_case]
	fn new_builtin_init_fail() {
		let result = Module::new_builtin("test", Version::new(0, 1, 0), init_fail, fini_noop);
		assert!(result.is_err());
	}

	#[test_case]
	fn new_builtin_init_ok() {
		let m = Module::new_builtin("test", Version::new(1, 2, 3), init_ok, fini_noop)
			.expect("new_builtin should succeed");
		assert_eq!(m.get_name(), b"test");
		assert_eq!(*m.get_version(), Version::new(1, 2, 3));
	}
}
