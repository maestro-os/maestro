...
use crate::{
	...
	syscall::{
		...
		epoll::{epoll_create, epoll_create1, epoll_ctl, epoll_pwait, epoll_wait},
		execve::execve,
		execve::execveat,
		fcntl::{fcntl, fcntl64},
		fd::{
		...
		select::{_newselect, compat_pselect6, compat_select, poll, pselect6, select},
		signal::{
			compat_rt_sigaction, compat_sigaltstack, kill, rt_sigaction, rt_sigpending,
			rt_sigprocmask, rt_sigreturn, rt_sigsuspend, rt_sigtimedwait, sigaltstack, signal,
			sigreturn, sigsuspend, tkill,
		},
		socket::{
			accept, accept4, bind, connect, getsockname, getsockopt, listen, sendto, setsockopt,
			shutdown, socket, socketpair,
		},
		stat::{
			fstat, fstat64, fstatat64, fstatfs, fstatfs64, lstat, lstat64, newfstatat, oldfstat,
			oldlstat, oldstat, stat, stat64, statfs, statfs64, statx,
		},
		sync::{fdatasync, fsync, msync, sync, syncfs},
		time::{
			clock_getres, clock_getres64, clock_gettime, clock_gettime64, getitimer, getitimer32,
			nanosleep32, nanosleep64, setitimer, setitimer32, time32, time64, timer_create,
			timer_delete, timer_settime, timer_settime64,
		},
		user::{
...
fn do_syscall32(id: usize, frame: &mut IntFrame) -> EResult<usize> {
	match id {
		...
		0x01c => syscall!(oldfstat, frame),
		// TODO 0x01d => syscall!(pause, frame),
		0x01e => syscall!(utime, frame),
		...
		0x048 => syscall!(sigsuspend, frame),
		...
		0x052 => syscall!(compat_select, frame),
		...
		0x068 => syscall!(setitimer32, frame),
		0x069 => syscall!(getitimer32, frame),
		...
		0x12b => syscall!(futimesat, frame),
		0x12c => syscall!(fstatat64, frame),
		0x12d => syscall!(unlinkat, frame),
		0x12e => syscall!(renameat, frame),
		...
		0x134 => syscall!(compat_pselect6, frame),
		...
		0x13f => syscall!(epoll_pwait, frame),
		...
		0x196 => syscall!(clock_getres64, frame),
...
fn do_syscall64(id: usize, frame: &mut IntFrame) -> EResult<usize> {
	match id {
		...
		0x082 => syscall!(rt_sigsuspend, frame),
		0x083 => syscall!(sigaltstack, frame),
		0x084 => syscall!(utime, frame),
		...
		0x105 => syscall!(futimesat, frame),
		0x106 => syscall!(newfstatat, frame),
		0x107 => syscall!(unlinkat, frame),
		0x108 => syscall!(renameat, frame),
		...
		0x119 => syscall!(epoll_pwait, frame),
		...
		0x144 => syscall!(membarrier, frame),
