...
{
	return parse_node(FIELD_FLAGS, src, len, 1, byte_data);
}

static aml_node_t *named_field(const char **src, size_t *len)
{
	return parse_node(NAMED_FIELD, src, len, 2, name_seg, pkg_length);
}
...
static aml_node_t *reserved_field(const char **src, size_t *len)
{
	...
	++(*src);
	--(*len);
	if(!(node = parse_node(RESERVED_FIELD, src, len, 1, pkg_length)))
	{
		*src = s;
		*len = l;
		return NULL;
...
static aml_node_t *access_field(const char **src, size_t *len)
{
	...
	++(*src);
	--(*len);
	if(!(node = parse_node(ACCESS_FIELD, src, len,
...
{
	return parse_node(FIELD_ELEMENT, src, len, 5, named_field, reserved_field,
		access_field, extended_access_field, connect_field);
}

aml_node_t *field_list(const char **src, size_t *len)
{
	return parse_list(FIELD_LIST, src, len, field_element);
}
