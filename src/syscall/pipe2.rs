...
pub fn pipe2(regs: &Regs) -> Result<i32, Errno> {
	...
	let (fd0_id, _) = proc.create_fd(open_file::O_RDONLY | flags, FDTarget::Pipe(pipe.clone()))?;
	let (fd1_id, _) = proc.create_fd(open_file::O_WRONLY | flags, FDTarget::Pipe(pipe.clone()))?;
