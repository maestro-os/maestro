...
use core::{alloc::AllocError, fmt, fmt::Formatter};

/// An alias to [`Result`] with [`AllocError`] as error type.
pub type AllocResult<T> = Result<T, AllocError>;

/// Wrapper for allocation result for the [`Iterator::collect`] function.
///
/// In the standard library of Rust, the `collect` function can return a [`Result`],
...
impl From<AllocError> for Errno {
	fn from(_: AllocError) -> Self {
		// FIXME on debug, location information are invalid
		crate::errno!(ENOMEM)
	}
}
...
/// An alias to [`Result`] with [`Errno`] as error type.
